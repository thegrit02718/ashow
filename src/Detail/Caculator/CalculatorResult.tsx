...
function CalculatorResult (props : any) {
  ...
  const contractCostPer = props.route.params.aptData.contractCostPer;
  const middleCostPer = props.route.params.aptData.middleCostPer;
  const restCostPer = props.route.params.aptData.restCostPer;
  const resultCost = props.route.params.aptData.resultCost;
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.section}>
        <View style={{flexDirection:'row', justifyContent: 'space-between', marginVertical: 10, alignItems: 'center' }}>
          ...
          <TouchableOpacity 
            onPress={()=>{props.navigation.navigate('CalculatorFilter', { resultCost : resultCost})}}
          >
      ...
      {/* notice 섹션 */}
      <View style={styles.section}>
        <Typography fontSize={22} color='#3D3D3D' marginBottom={5} fontWeightIdx={2}>
          {asyncGetData.userNickName}님의 <Typography fontSize={24}>자금스케줄</Typography>이
        </Typography>
        <Typography fontSize={22} color='#3D3D3D' marginBottom={5} fontWeightIdx={2}>완성되었어요!</Typography>
      </View>
        ...
        <View style={{ width: '100%', height: '100%', position: 'absolute', alignItems: 'center', justifyContent: 'center'}}>
          <View style={{borderRadius: 20, backgroundColor: 'white', padding: 20, width: '90%'}}>
            <View style={styles.textBox}>
              ...
              {/* <TouchableOpacity 
                onPress={()=>{
                  props.navigation.navigate('CalculatorFilter', { resultCost : props.route.params.resultCost});
                  summuryToggleModal();
                }}>
      ...
      <View style={styles.section}>
        ...
        <View style={{borderWidth:1, borderColor:'#DFDFDF', borderRadius:10, padding: 20}}>
          <View style={styles.textBox}>
            ...
            <Typography fontSize={18}>{FormatNumber(props.route.params.resultCost)}</Typography>
          </View>
          <Divider height={1} marginVertical={10}/>

          <View style={styles.textBox}>
            <Typography fontSize={14} color='#8B8B8B' >계약금</Typography>
            <Typography fontSize={14}>{FormatNumber(0)}</Typography>
          </View>
          <View style={styles.textBox}>
            <Typography fontSize={14} color='#8B8B8B' >중도금</Typography>
            <Typography fontSize={14}>{FormatNumber(0)}</Typography>
          </View>
          <View style={styles.textBox}>
            <Typography fontSize={14} color='#8B8B8B' >잔금</Typography>
            <Typography fontSize={14}>{FormatNumber(0)}</Typography>
          </View>
          <View style={styles.textBox}>
            <Typography fontSize={14} color='#8B8B8B' >발코니확장비</Typography>
            <Typography fontSize={14}>{FormatNumber(0)}</Typography>
          </View>
          <View style={styles.textBox}>
            <Typography fontSize={14} color='#8B8B8B' >대출금</Typography>
            <Typography fontSize={14}>{FormatNumber(props.route.params.loanCost)}</Typography>
          </View>
          <View style={styles.textBox}>
            <Typography fontSize={14} color='#8B8B8B'>대출 월납입 예상금액</Typography>
      ...
      <View style={{padding: 20}}>
        <Shadow distance={5} startColor="#dfdfdf" style={{ borderRadius:10, overflow:'hidden', width:'100%'}}>
